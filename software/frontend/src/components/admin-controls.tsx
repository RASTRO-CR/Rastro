...
export function AdminControls({ raceStarted, onStartRace, onEndRace, onRouteUploaded }: AdminControlsProps) {
  ...
  const calculateElevationGain = (xmlDoc: Document) => {
    const elevations = Array.from(xmlDoc.querySelectorAll("ele")).map((el) => Number.parseFloat(el.textContent || "0"))
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 h-[calc(100%-60px)]">
          ...
          <div className="space-y-3">
            ...
            {gpxFile && (
              <div className="text-sm space-y-1">
                ...
                <div className="text-gray-300">Size: {(gpxFile.size / 1024).toFixed(1)} KB</div>
              </div>
            )}

            {routeData && (
              <div className="text-sm space-y-1 p-2 bg-white/10 rounded">
                <div className="text-blue-400 font-semibold">{routeData.name}</div>
                <div className="text-gray-300">Distance: {routeData.distance.toFixed(1)} km</div>
                <div className="text-gray-300">Elevation: +{routeData.elevationGain.toFixed(0)}m</div>
                <div className="text-gray-300">Points: {routeData.coordinates.length}</div>
              </div>
            )}
        ...
        <div className="backdrop-blur-md bg-white/10 border border-white/20 rounded-xl p-4">
          ...
          <div className="space-y-3">
            ...
            <Button onClick={onEndRace} disabled={!raceStarted} variant="destructive" className="w-full">
              <Square className="w-4 h-4 mr-2" />
              End Race
            </Button>

            {!routeData && (
              <div className="text-xs text-yellow-400 text-center">Upload and process a GPX route first</div>
            )}
          </div>
        </div>
